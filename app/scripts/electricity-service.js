...
angular.module('fuelCalculator').service('electricityService', [
  'unitService',
  function (unitService) {
    var TARIFFS = {
      metropolitan: { regular: [[6000, 0.18]], excessive: 0.3 },
      profit: { regular: [[6000, 0.2]], excessive: 0.3 },
      government: { regular: [], excessive: 0.32 },
      enterprise: { regular: [], excessive: 0.18 },
      agriculture: { regular: [[6000, 0.16]], excessive: 0.2 }
    }
    ...
    this.getPeriodTable = function () {
      ...
      return [[MONTH, 1], [THREE_MONTHS, 3], [SIX_MONTHS, 6], [YEAR, 12]]
    }
    ...
    this.calculateConsumptionByCost = function (category, meter, cost) {
      ...
      return unitService.calculateInvertedUnit(
        tariff.regular,
        tariff.excessive,
        electricityCost
      )
