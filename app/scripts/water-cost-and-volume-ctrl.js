...
angular.module('fuelCalculator').controller('WaterCostAndVolumeCtrl', [
  '$routeParams',
  'fuelService',
  'menuService',
  'periodService',
  ...
  function (
    ...
    this.measureMenu = menuService.getWaterMeasureMenu(
      validationService.getValidWaterMeasure($routeParams.measure)
    )
    this.periodMenu = menuService.getWaterPeriodMenu(
      validationService.getValidWaterPeriod($routeParams.period)
    )
    this.periodTable = periodService.getWaterTable()

    this.calculateVolume = function (factor) {
      var measure = vm.measureMenu.getSelected()
      var costOrVolumePerMonth = vm.periodMenu.getValuePerMonth(
        vm.costOrVolume
      )
